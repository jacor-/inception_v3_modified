...
int main(int argc, char* argv[]) {
  ...
  //LOG(INFO) << FLAGS_output_layer;
  //LOG(INFO) << FLAGS_print_last_layer;
  //LOG(INFO) << FLAGS_image;
  if(strcmp("softmax",FLAGS_output_layer.c_str())) {
    LOG(INFO) << "FEATURES " << " " << FLAGS_image << " " << FLAGS_output_layer << " " << outputs[0].SummarizeValue(5000) ;
  }
  else {
